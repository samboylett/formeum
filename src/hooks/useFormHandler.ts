...
export interface UseFormHandlerArg<Values> {
  ...
  validate?: (values: Values, fieldName?: O.Paths<Values>) => Promise<FormErrors<Values>>;
}
...
export interface UseFormHandlerReturn<Values> {
  ...
  events: EventEmitter;
  setFieldValue: <Name extends string & O.Paths<Values>>(name: Name, value: DeepIndex<Values, Name>, shouldValidate?: boolean) => void;
...
export const createUseFormHandler = <Values>() => {
  const useFormHandler = ({ initialValues, validate }: UseFormHandlerArg<Values>): UseFormHandlerReturn<Values> => {
    ...
    const setFieldValue = useCallback(async <Name extends string & O.Paths<Values>>(name: Name, value: DeepIndex<Values, Name>, shouldValidate?: boolean) => {
